...
public class DownloadServletTest {
    ...
    @Test
    public void customAccessTokenHeaderIsRespected() throws Exception {
        DownloadServlet secured = securedDownloadServlet("secret");
        secured.setAccessTokenHeader("X-API-Token");

        MockHttpServletRequest denied = downloadRequest();
        MockHttpServletResponse deniedResponse = new MockHttpServletResponse();
        secured.doGet(denied, deniedResponse);
        assertEquals(401, deniedResponse.getStatus());

        MockHttpServletRequest ok = downloadRequest();
        ok.addHeader("X-API-Token", "secret");
        MockHttpServletResponse okResponse = new MockHttpServletResponse();
        secured.doGet(ok, okResponse);
        assertEquals(200, okResponse.getStatus());
        assertArrayEquals(CONTENT, okResponse.getContentAsByteArray());
    }
    ...
    private DownloadServlet securedDownloadServlet(String token) throws Exception {
        UploadFileContext.Config config = new UploadFileContext.Config();
        config.securityEnabled = true;
        ...
        request.setChunkTotal(1);
        request.setChunkIndex(0);
        uploadService.uploadChunk(request, token, new ByteArrayInputStream(CONTENT));
        uploadService.merge(IDENTIFIER, token);
